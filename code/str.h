...
public:
    ...
    int                 sprintf_nogrow(const char* fmt, ...);
    int                 vsprintf_nogrow(const char* fmt, va_list args);
    int                 append(char c);
...
{                                                                                   \
public:                                                                             \
    TYPENAME_F(const char* fmt, ...) : TYPENAME() { va_list args; va_start(args, fmt); this->vsprintf(fmt, args); va_end(args); } \
};
...
int     Str::vsprintf(const char* fmt, va_list args)
{
...
int     Str::sprintf(const char* fmt, ...)
{
    va_list args;
    va_start(args, fmt);
...
int     Str::vsprintf_nogrow(const char* fmt, va_list args)
{
    STR_ASSERT(Owned);
...
int     Str::sprintf_nogrow(const char* fmt, ...)
{
    va_list args;
    va_start(args, fmt);
    int len = vsprintf_nogrow(fmt, args);
    va_end(args);
    return len;
}
